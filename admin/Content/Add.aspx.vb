...
Partial Class admin_Content_Add
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If String.IsNullOrEmpty(Id.Value) Then
            cmd = New SqlCommand("Insert Into RCContent ([Name], [Value], [PageId]) Values (@Name, @Value, @PageId)", con)
        Else
            cmd = New SqlCommand("Update RCContent set [Name]=@Name, [Value]=@Value, [PageId]=@PageId, [DateModified]=GetDate() where Id=@Id", con)
    ...
    End Sub
     ...
     Protected Sub btnSaveContinueEdit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveContinueEdit.Click
        ...
        If String.IsNullOrEmpty(Cid) Then
            cmd = New SqlCommand("Insert Into RCContent ([Name], [Value], [PageId]) Values (@Name, @Value, @PageId); SELECT SCOPE_IDENTITY()", con)
        Else
            cmd = New SqlCommand("Update RCContent set [Name]=@Name, [Value]=@Value, [PageId]=@PageId, [DateModified]=GetDate() where Id=@Id", con)
            cmd.Parameters.AddWithValue("Id", CInt(Id.Value))
        End If
